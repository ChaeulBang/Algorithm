...
Node* restructure(Node** root, Node* x, Node* y, Node* z);	//3-노드 개조
...
void searchAndFixAfterRemoval(Node** root, Node* node);	//균형검사를 수행하고 불균형이 있으면 개조를 통해 높이균형 속성 회복
...
Node* reduceExternal(Node** root, Node* z) {
	...
	else {	//w가 루트 노드가 아닐 경우
		g = w->parent;	//g는 w의 부모
		zs->parent = g;	//zs를 g의 자식으로 승진시킴
...
int removeElement(Node** root, int k) {
	Node* w = treeSearch(*root, k);	//현재 트리에서 삭제할 키 값을 찾음
	int e = w->key;
	Node* z = w->lChild;	//z는 w의 왼쪽 자식
	Node* zs, * y;	

	if (isExternal(w)) return 0;	//현재 트리에 없는 키라면 종료

	//현재 트리에 있는 키라면
	if (!isExternal(z)) z = w->rChild;	//z가 내부노드라면 z는 w의 오른쪽 자식
	if (isExternal(z)) zs = reduceExternal(root, z);	//삭제 후 그 형제를 반환
	else {
		y = inOrderSucc(w);	//y는 w의 중위순회 후계자
		z = y->lChild;	//z는 y의 왼쪽 자식 (외부 노드)
		w->key = y->key;	//y의 키 값을 w의 키 값으로 대체
		zs = reduceExternal(root, z);	//삭제 후 그 형제를 반환
	}

	searchAndFixAfterRemoval(root, zs->parent);	//반환된 노드의 부모부터 균형 검사 시작
...
void searchAndFixAfterRemoval(Node** root, Node* w) {
	...
	if (w == NULL) return;	//루트 노드일 경우
	z = w;	//루트 노드가 아닐 경우 z를 w의 부모로 설정
	updateHeight(z);	//높이 균형 속성 업데이트
	...
	searchAndFixAfterRemoval(root, b->parent);	//높이 균형 속성을 전역적으로 복구하기 위해 재귀 호출
}